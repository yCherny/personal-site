...
function About({session}: Props) {
  ...
  const [filteredContent, setFilteredContent] = useState<
    ContactContent[] | null
  >();
  const [allContactFilters, setAllContactFilters] = useState<string[] | null>();
  ...
  function filterContent(type: string) {
    if (type === filter) {
      ...
      const filteredContent = allContacts?.filter((data) => data.type === type);
      setFilter(type);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 md:gap-10 mt-5">
          ...
          <div className="col-span-2">
            <div className="flex flex-col gap-5">
              {filteredContent
                ? filteredContent.map((contact: ContactContent) => (
                    <ContactCard
                      contact={contact}
